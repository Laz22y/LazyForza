...
3. 在工具中选择当前地图和采集批次，点击“开始后台采集并最小化”。工具会立即最小化，让 FH5 回到前台并恢复运行。
4. 完整驾驶一段覆盖地图不同区域的路线。在入口、中心、边缘、高点和低点等位置停车约一秒，然后按 `Ctrl+Shift+F8` 记录地标；地标按窗口中填写的名称前缀自动编号。
5. 驾驶完成后直接在 FH5 前台按 `Ctrl+Shift+F9`。工具会在后台停止并保存，无需切回工具窗口。每张地图和每次重启分别生成一份 `.fh5mapcapture` 文件。

FH5 在失去焦点后会暂停，因此采集期间不需要操作工具窗口。地标记录成功、记录失败和保存完成会播放不同的系统提示音。普通暂停或菜单包仍保留在原始包中供协议核查，但不会进入驾驶坐标范围，也不会用于地标计算。若全局快捷键已被其他软件占用，工具会在开始前提示，不会进入无法结束的后台采集。
