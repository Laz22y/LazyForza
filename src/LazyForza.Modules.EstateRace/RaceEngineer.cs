...
/// <summary>Implementations must stop speech and radio cues when cancellation is requested.</summary>
public interface ILocalRaceSpeech
{
    Task SpeakAsync(string text, int volume, CancellationToken cancellationToken);
}

/// <summary>Bounded, replaceable local output. No race or telemetry producer waits for audio.</summary>
public sealed class RaceEngineer : IDisposable
{
    private readonly object sync = new();
    private readonly ILocalRaceSpeech speech;
    ...
    public RaceEngineer(ILocalRaceSpeech speech, Func<DateTimeOffset>? clock = null)
    {
        this.speech = speech;
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    lock (sync)
                    {
                        ...
                        if (queue.Count == 0 || !enabled || muted || volume == 0 || error is not null) break;
                        ...
                        level = volume;
                        if (message.Cooldown > TimeSpan.Zero) lastSpoken[message.Category] = clock();
                    }
                    try { await speech.SpeakAsync(message.Text, level, cancellation.Token).ConfigureAwait(false); }
                    catch (OperationCanceledException) when (cancellation.IsCancellationRequested) { }
                    catch (Exception)
                    {
                        lock (sync)
                        {
                            if (!cancellation.IsCancellationRequested) { error = "Local speech unavailable"; queue.Clear(); }
                        }
                    }
                    finally
                    {
                        lock (sync) { current = null; playing = null; }
    ...
    private void ClearPlayback() { queue.Clear(); playing?.Cancel(); }
