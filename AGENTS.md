...
- 普通开发不生成发行、不推送。开发预览使用 `scripts/New-DevPreview.ps1`；便携版必须通过首次初始化选择数据目录，不再附带强制隔离数据的启动脚本。预览包使用独立初始化状态和 GitHub 预发布更新通道，每次启动强制检查并自动安装可用更新；正式版仍使用原有 GitCode/GitHub 可配置更新流。
- 只有用户明确要求分发预览版时才运行 `scripts/Publish-Preview.ps1`。该脚本只创建 GitHub Prerelease 和预览便携包，不更新 GitCode、正式版下载入口或稳定版版本号。
