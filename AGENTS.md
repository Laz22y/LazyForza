...
- 普通开发不生成发行、不推送。开发预览使用 `scripts/New-DevPreview.ps1`；便携版必须通过首次初始化选择数据目录，不再附带强制隔离数据的启动脚本。预览包使用独立初始化状态和 GitCode/GitHub 预发布更新通道，每次启动强制检查并自动安装可用更新；正式版仍使用原有稳定版更新流和独立来源设置。
