...
- 默认用户数据位于 `%LOCALAPPDATA%\LazyForza`；`--data-dir` 可隔离。不得把数据库、录制、日志、设置或备份打进源码提交/发行包。
- 保留工作树中不属于当前任务的修改。不要用重置、覆盖或大范围格式化处理它们。
- 普通开发不生成发行、不推送。开发预览使用 `scripts/New-DevPreview.ps1`，并确认包内 `Start-Development.cmd` 以相邻 `LazyForza_Data` 启动。
