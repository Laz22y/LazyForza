(function initializeWebsiteLocalization(globalObject) {
  ...
  const ENGLISH = {
    "预览版 1.5.3-alpha-3 · Radio Check": "Preview 1.5.3-alpha-3 · Radio Check",
    "RaceServer 后续开发版提供项目与规则管理、逐场记录和主动退出释放。该功能需要配套服务端，部署前请查看其版本兼容说明。": "Upcoming RaceServer builds provide project and rule management, per-event records and slot release on departure. These features require a matching server build; check its version compatibility before deployment.",
    "RaceServer 后续开发版按比赛现场、赛事项目、规则与赛程、赛果与记录、服务器划分总控。一项目对应一场赛事；准备下一场时归档旧赛果并清理实时成绩和离线占位。配套客户端主动退出后释放席位；暂时掉线保留恢复身份，大厅离线占位五分钟后释放。使用前请核对服务端仓库的长期房间与版本兼容说明。": "Upcoming RaceServer builds organize control into the live event, projects, rules and schedule, results and records, and server settings. Each project represents one event; preparing the next archives previous results and clears live standings and offline slots. A matching client releases its slot when leaving. Temporary disconnections retain recovery identities, with offline lobby slots released after five minutes. Check the server repository's persistent-room and version compatibility guidance before use.",
    "语音工程师（预览版）": "Race engineer (preview)",
    ...
    "窗口与设置（预览版）": "Window and settings (preview)",
    ...
    "1.5.3-alpha-3 · Radio Check：新增六家在线语音服务、Windows 音色选择和提示音独立开关，更新品牌顶栏与侧栏，设置修改后即时应用，并改善地产 HUD 显示与维修区计圈。预览版使用独立更新通道，推荐搭配 RaceServer 0.6.0-alpha-1（协议 v2）。": "1.5.3-alpha-3 · Radio Check adds six online speech providers, Windows voice selection and independent radio cues, refreshes the title bar and sidebar, applies settings automatically, and improves estate HUD rendering and pit-lane lap counting. Previews use a separate update channel; RaceServer 0.6.0-alpha-1 is recommended (protocol v2).",
    ...
    "了解首次连接、驾驶分析、语音配置、赛事服务端部署与开发。": "Learn about first-time setup, driving analysis, speech configuration, race hosting and development.",
    "客户端": "Client",
    "数据入口": "Data input",
    "数据来自 FH6 官方 UDP Data Out。赛道识别和驾驶建议由 LazyForza 根据遥测推导，并标明数据来源。": "Data comes from official FH6 UDP Data Out. LazyForza derives track recognition and driving guidance from telemetry and identifies their data sources.",
    ...
    "根据本车 UDP 遥测显示侧滑角、控车趋势和失控风险提示。换挡建议侧重维持漂移，得分以游戏内显示为准。": "Shows slip angle, control trends and spin-risk guidance from local UDP telemetry. Shift guidance focuses on sustaining a drift; use the in-game display for scores.",
    ...
    "主动连接 RaceServer 后，赛事所需资料和遥测摘要发送到该服务器；": "Connecting to RaceServer sends the required race details and telemetry summaries to that server;",
    "官方字段与 LazyForza 推导的赛道识别、驾驶分析分别说明来源；": "Official fields and LazyForza-derived track recognition and driving analysis identify their respective sources;",
    "录制容量、轮换、备份和更新均由用户控制。": "Recording limits, rotation, backups and updates remain under user control.",
    "更新": "Updates",
    "正式安装版默认启动检查更新，正式便携版默认关闭，两者都可在设置中修改。正式版只提示、不强制安装，下载优先使用 GitCode，失败时回退 GitHub。预览版使用独立初始化状态和 GitCode/GitHub 预发布更新通道，每次启动强制检查并只自动安装更高预览版；正式版不参与预览通道，预览包不会自动转为正式版。可独立选择首选来源，失败时自动切换备用来源。检查或安装失败时需重试或退出。安装前均校验发行包 SHA-256 和包内清单，失败时恢复原版本。": "Stable installed builds check for updates by default; stable portable builds do not, and both settings can be changed. Stable updates are offered rather than forced, with GitCode preferred and GitHub as fallback. Preview builds use separate initialization state and GitCode/GitHub prerelease channels, check on every startup, and install only newer previews automatically. Stable releases never enter the preview channel, so preview builds do not automatically become stable. Their preferred source is configured separately and falls back automatically. A failed check or installation must be retried or the app exits. Every package verifies SHA-256 and its internal manifest before installation, with rollback on failure.",
    ...
    "和": "and",
    "默认监听": "Listens on",
    "全新原生服务器必须先在服务器交互终端运行": "A new native server must first run",
    "，设置房间密码、初始超级管理员密码和现有基础规则；密码不会回显。初始化完成后再启动服务，默认监听": "in an interactive server terminal to set the room password, initial super-admin password and existing base rules; passwords are not echoed. Start the service after initialization. It listens on",
    "。未初始化时正常启动会在监听端口前退出，浏览器和远程 API 不能完成原生首次设置。已有有效凭据的服务器升级后直接继续使用原配置。公网部署应由 Caddy、Nginx 或同类反向代理终止 TLS，让客户端连接": "by default. An uninitialized normal start exits before listening on any port; browsers and remote APIs cannot perform native first setup. Upgraded servers with valid credentials continue using their existing configuration. For public hosting, terminate TLS with Caddy, Nginx or a similar reverse proxy so clients connect over",
    "。": ".",
    ...
    "检查地址、房间密码和协议版本。公网环境需要 HTTPS/WSS，反向代理需转发 WebSocket Upgrade。网页可访问但客户端连接失败时，检查代理的 Upgrade 配置和 WebSocket 连接日志。": "Check the address, room password and protocol version. Public hosting needs HTTPS/WSS and WebSocket Upgrade forwarding. If the webpage opens but the client cannot connect, check the proxy's Upgrade configuration and WebSocket logs.",
    "更新失败": "Update failed",
