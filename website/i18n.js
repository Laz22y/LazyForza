(function initializeWebsiteLocalization(globalObject) {
  ...
  const ENGLISH = {
    ...
    "和": "and",
    "默认监听": "Listens on",
    "。首次打开网页时设置房间密码、总控密码和赛事规则。公网部署应由 Caddy、Nginx 或同类反向代理终止 TLS，让客户端连接": "by default. On first launch, set the room password, Race Control password and race rules. For public hosting, terminate TLS with Caddy, Nginx or a similar reverse proxy so clients connect over",
    "。": ".",
    ...
    "车手需要服务端域名或 IP、房间密码、匹配的地产赛道、显示名和可选车队。OB 只接收赛事快照，不上传遥测、不参与排名和处罚。": "Drivers need the server domain or IP, room password, matching estate circuit, display name and optional team. Observers receive race snapshots only; they do not upload telemetry or participate in standings or penalties.",
    "兼容性": "Compatibility",
