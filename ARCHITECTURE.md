...
待发圈按赛道、有效阶段、练习／排位节次与计时标识隔离；新阶段、新车手或主动重新连接清理旧队列，同阶段自动重连、暂停恢复及结束后的回执等待保留队列。发送前再次检查事件是否仍待确认。此行为沿用协议 v2 的 `lapCompleted`／`lapAcknowledged`，不增加 Schema 字段，兼容现有原生与 Cloudflare 服务端；最终是否计入成绩仍由服务端规则决定。阶段隔离依据已收到的快照，既有协议不能撤回已发往服务端的消息，也不能识别尚未收到的阶段切换。
