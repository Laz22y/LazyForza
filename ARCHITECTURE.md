...
历史圈从 SQLite 恢复后按路线、方向和 `SectorSchemaVersion` 限定比较范围。模块常驻内存的历史圈只保存 `LapSummary`（元数据和分段），不加载高密度 `LapSamples`。圈速分析默认将元数据按会话、赛道修订、方向、分段版本与玩家分组；性能等级筛选匹配整个场次，整场统计不截取其中部分圈。记录总用时包含所有有限正数圈时，有效圈中位数和标准差只统计有效圈。单圈页签按需创建，UI 勾选 0–4 圈后点击“查看对比”批量加载采样；模块只缓存最近使用的 8 个完整圈。速度曲线使用保留峰谷的包络降采样和冻结几何，悬停命中继续使用按进度二分；走线图缓存绘制和屏幕空间命中网格。每条赛道默认以 500 圈为保留目标，可设置为 1–10,000 圈。自动清理保护收藏、固定参考，以及同车型、性能等级、PI、驱动、气缸数和兼容可观测转速／齿比／动力特征下的最快有效代表圈；路线修订、方向、分段与玩家各自隔离。保护圈所在场次和最近场次整体保留，再补充最近场次，因此保护与完整场次可使数量超过目标。缺少会话 ID 的旧记录按独立条目保留。
...
当前 `LazyForzaStore.CurrentSchemaVersion` 为 15。基础表包含 AppSettings、ModuleSettings、Sessions、VehicleProfiles、EngineCurveBins、GearModels、ShiftTargets、TrackTemplates、TrackPoints、SectorDefinitions、Laps、LapSegments 与 LapSamples；后续迁移加入动态遥测、地产赛道定义/检查点/维修区/计时类型、地产策略样本和圈速玩家代号。圈速显式保存官方 CarClass/PI；升级前缺失的 CarClass 会先从旧车辆指纹恢复 PI，再按 D 100–400、C 401–500、B 501–600、A 601–700、S1 701–800、S2 801–900、R 901–998、X 999 的规范区间补齐。
...
Schema 14 追加可空的 `Laps.SessionInfo` 和赛道／会话索引。新记录区分普通比赛、地产独立计时、练习、排位和正赛。地产记录使用已有服务端阶段与车手身份生成稳定的本地分析 ID，同阶段重连或暂停恢复继续归入原场次；旧服务端缺少阶段 ID 时仅保持本次连接内的归属。普通地图使用客户端已有的比赛会话识别，不代表 FH6 官方赛事 ID。旧记录仅依赖原有 SessionId，不能补猜阶段，也不能恢复曾被清理的圈。备份、圈速交换文件和单圈回放保留可选 SessionInfo；旧文件缺失时按空读取，网络协议不变。
...
Schema 15 在 `Laps.Annotation` 中保存可选的 `LapAnnotation`（名称、备注、收藏、固定参考），不修改计时和样本。元数据更新与同条件参考替换在单个事务内完成；自动清理和圈保存共用写入锁，避免清理时遗漏刚保存的收藏。保留目标存于 AppSettings 的 `lapLibrary.capacityPerTrack`，修改本身不删除数据，从下一次保存起执行。列表虚拟化展示，空间统计在后台通过 SQL 聚合与 SQLite 页数读取，不加载所有样本。圈数据大小是有效载荷估算；数据库占用含其他表、索引与可复用空闲页。
