...
圈速对比、赛后复盘和回放工作台支持手动弯道分析：在走线预览上依次点击起点和终点，高亮所选区间，也可输入距离微调。默认「仅分析本圈」，显示区间耗时、制动起点、最低速度及恢复油门位置；可从全部保留历史中选择兼容的有效参考圈，按赛道距离比较差异。走线支持滚轮缩放、拖动平移和双击复位；最多显示三条差异说明，点击即可定位曲线。区间标记保存在本机，每个赛道版本最多 32 个；无法确认路线版本的记录按单圈保存标记，跨终点弯请拆成两段。
