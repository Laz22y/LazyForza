...
侧栏底部显示遥测状态与当前 UDP 端口，模拟／回放模式显示对应来源。设置 → 常规 → 快速设置可选择显示语音工程师静音、HUD 换挡提示、本车换挡推荐、语音音量和减少动态，默认显示静音与 HUD 换挡提示，已有自定义列表保持原有选择。HUD 换挡提示控制主仪表盘与漂移 HUD 的提示符号显示，可随时切换；本车换挡推荐控制当前车辆配置是否提供升降挡建议，与「车辆与换挡」页同步，车辆配置就绪后可用。两项独立保存，隐藏符号时继续学习车辆数据。较短窗口将多个快速入口合并为图标行，设置入口固定在底部。
