...
侧栏底部显示遥测状态与当前 UDP 端口，模拟／回放模式显示对应来源。设置 → 常规 → 快速设置可选择显示语音工程师静音、推荐换挡、语音音量和减少动态，默认显示静音与推荐换挡，已有自定义列表保持原有选择。推荐换挡切换当前车辆配置的升降挡提示，与「车辆与换挡」页同步；尚未识别车辆时不可操作。选择和状态在重启后保留。较短窗口将多个快速入口合并为图标行，设置入口固定在底部。
