...
| 圈速分析 | 已保存圈速与分段、速度与驾驶输入、走线和距离游标联动 |
...
圈速对比、赛后复盘和回放工作台支持手动弯道分析：在走线预览上依次点击起点和终点，高亮所选区间，也可输入距离微调。有兼容的固定参考时优先使用，否则默认「仅分析本圈」，显示区间耗时、制动起点、最低速度及恢复油门位置；可从全部保留历史中选择兼容的有效参考圈，按赛道距离比较差异。走线支持滚轮缩放、拖动平移和双击复位；最多显示三条差异说明，点击即可定位曲线。区间标记保存在本机，每个赛道版本最多 32 个；无法确认路线版本的记录按单圈保存标记，跨终点弯请拆成两段。

圈速分析默认按比赛整理记录。地产练习、排位、正赛和独立计时分别归档，普通地图按本地识别的比赛会话归档；选择场次可查看记录总用时、典型圈速、波动和逐圈节奏，再打开某圈深入分析。「单圈对比」通过「选择对比圈」切换最多四圈。圈记录支持收藏、命名、备注和固定参考；可按名称、备注或车型搜索，也可只看收藏。默认每赛道保留目标为 500 圈，可在「设置 → 遥测与录制 → 圈记录存储」自定义为 1–10,000 圈，并查看空间用量。自动清理保护收藏、固定参考及各车型与可观测车辆条件下的代表圈，保留其完整场次；受保护记录与完整比赛可能超出目标。旧记录按已有会话整理，缺失圈不会补全；记录总用时不包含缺失圈和服务端处罚。「曲线」「走线」「弯道」使用统一页签；导入、导出和记录管理常驻页面顶部，实时分段集中在「当前比赛」显示。回放的播放、时间轴与读数集中显示；弯道编辑收起后保留草稿，切换视图仍保持距离游标联动。
...
语音默认使用 Windows 已安装的对应语言本地 SAPI 语音，无需联网；每次完整播报前后有原创的无线电接通／断开音，包含短双脉冲、轻微扫频和电台噪声尾音。语音准备好后才播放接通音，静音或中断会立即停止本次输出。缺少语音或音频不可用时只停用播报，比赛照常进行。安装对应语音后，可关闭再启用工程师以重试。合成与播放已通过独立接口分离，内置 Windows 本地语音及可选的 ElevenLabs、Azure Speech、腾讯云、阿里云智能语音交互、千问 AI 平台和 MiniMax API；开发接口见 [语音输出说明](docs/RACE_ENGINEER_SPEECH.md)。
...
侧栏底部显示遥测状态与当前 UDP 端口，模拟／回放模式显示对应来源。设置 → 常规 → 快速设置可选择显示语音工程师静音、HUD 换挡提示、本车换挡推荐、语音音量、HUD 整体不透明度、地产赛事底板不透明度、比赛自动录制和减少动态，默认显示静音与 HUD 换挡提示，已有自定义列表保持原有选择。音量与不透明度通过滑块浮窗调整，与设置页同步并自动保存；关闭比赛自动录制时会保存当前自动录制。HUD 换挡提示控制主仪表盘与漂移 HUD 的提示符号显示，可随时切换；本车换挡推荐控制当前车辆配置是否提供升降挡建议，与「车辆与换挡」页同步，车辆配置就绪后可用。两项独立保存，隐藏符号时继续学习车辆数据。较短窗口或选择较多项目时，快速入口合并为图标行，设置入口固定在底部。
