...
`serverVersion` records the native server product version at that revision. The optional host component uses that version plus a separate host revision (for example, `0.6.0-r1`); the internal NuGet version remains a commit-pinned package identity. Engine and control provenance must agree before packaging.
