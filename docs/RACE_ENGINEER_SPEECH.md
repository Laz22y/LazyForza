...
地产赛事页常驻三段式密度选择，「播报类别」和「最近播报」默认折叠。默认均衡密度、四类全部开启；精简只自动播报旗语和处罚，均衡对个人最快圈／进站建议分别使用 20／60 秒冷却，详细缩短至 10／30 秒。三个档位均遵守类别开关，旗语与处罚的优先级不变。关闭类别会清除对应待播消息并取消当前播报，恢复后不补播。

`EngineerPreferences` 通过 AppSettings 的 `raceEngineer.broadcast.v1` 保存密度和四个类别开关，无需迁移数据库；旧应用忽略新增设置，缺失或损坏的数据使用默认值，未知密度回退均衡。

最近记录最多保留 20 次实际输出请求，包括手动重播、正在播报、中断和失败的状态；试听不计入。记录只在内存保存，重启或重新建立语音输出（切换服务、音色）时清空，赛事切换后旧记录仍可阅读但不能重播。

「重复上一条」只接受最新一条已完整播出且仍有效的记录，不跳过失效记录去播放更早的消息。手动重播可跳过自动播报的密度和冷却，但仍遵守启用、静音、音量及类别开关；沿用原消息有效期（圈速／进站 15 秒，旗语／处罚 30 秒），不会重新计时。真实赛事消息优先于手动重播，忙碌时不可重复提交。

观察器为消息绑定赛事阶段、本车及原始状态。重播受理、出队时直接核对模块最新快照；合成与播放期间持续检查并在状态变化后取消。旗语切换、阶段变化、断线，以及处罚修改、执行、撤销或移除都会使对应旧消息失效，即使新消息被静音、类别开关或冷却过滤也一样。历史记录显示失效状态，重播按钮说明不可用原因。
...
每个输出实例只在内存中缓存成功合成的 PCM，最多 32 条或 4 MiB，按进入顺序淘汰；不写音频缓存文件。缓存绑定该实例的声音设置，播放时应用当前音量；失败结果不缓存，释放时清空。缓存保存的是可复用声音，自动播报仍执行事件去重，手动重播必须通过最新赛事状态和有效期检查。
...
播报偏好与重播回归还覆盖密度冷却、类别即时取消、20 条历史上限、手动重播不延长有效期、最新快照在界面刷新前失效，以及处罚执行／撤销／移除／修改。`EngineerBroadcastPanelTests` 检查偏好持久化兼容、默认折叠、按钮状态及中英文窄宽布局；离屏检查不代表真实比赛或音频设备验证。
