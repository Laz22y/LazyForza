...
`1.5.3-alpha-2` 为当前预览版，包含以下分析界面、弯道和语音工程师功能；地产赛事推荐配合 RaceServer `0.6.0-alpha-1`。协议保持 v2，旧服务端不提供新的重启恢复、圈校验和限速能力。预览版通过独立通道更新，正式下载仍为 `1.5.2`。
...
当前源码提供「Windows 音色」选择，显示本机 SAPI 可用的中英文声音；默认跟随界面语言，选择英文音色时播报英文。音色保存后下次启动恢复；切换会停止旧声音并清理待播消息，保存的音色不可用时回退默认并提示。
...
当前源码还支持独立开关接通音和断开音；关闭后跳过对应停顿，已导入音频继续保留。「试听」使用当前音色、音量和提示音开关。
