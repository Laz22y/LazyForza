...
- 车队、维修区、进站规则、旗语、处罚、碰撞调查和可选断线计圈恢复；
...
RaceServer `0.4.1` 支持 LazyForza `1.4.2–1.4.8` 的协议 v2 主要赛事流程。增强碰撞证据需要客户端 `1.4.7` 或更高版本；断线计圈恢复需要客户端 `1.4.8`，并由服务端总控主动开启。旧版仍可连接，但不会拥有后续新增的全部能力。
