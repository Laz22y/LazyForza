...
开发版另提供「直连房间 · 实验」：房主新建或打开保存的项目，其他成员粘贴完整邀请代码并输入密码加入。TCP 不可达时，支持 QUIC 的 Windows 11 环境可交换连接回执尝试双向 UDP。房主可在本机打开与服务端相同的 Web 总控。赛事通信仅发生在成员与房主之间，无目录、发现或中继服务；仍需可达的局域网、IPv6 或 IPv4 路径。加入功能内置，房主组件单独安装，当前支持匹配离线包。它同步 LazyForza 赛事数据，游戏车队仍在 FH6 内建立。详见[直连房间说明与网络测试](ESTATE_PEER_ROOMS.md)。
