...
正式安装版默认启动检查更新，正式便携版默认关闭，两者都可在设置中修改。正式版发现新版时先由用户确认，不强制安装；更新说明按当前界面语言显示中文或英文。程序优先使用 GitCode，失败时回退 GitHub：安装版下载并校验新版安装程序，完成系统安装信息和卸载清单升级；便携版校验 ZIP 的 SHA-256 与包内清单后原地更新，失败时恢复原版本。预览版使用独立初始化状态和 GitHub 预发布更新通道，每次启动强制检查；发现更高预览版或对应正式版时自动下载、校验、替换并重启，检查或安装失败时需重试或退出。正常升级不会改变已选择的数据目录或重复执行初始化。
