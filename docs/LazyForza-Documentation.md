...
在「弯道」页签选择分析圈与真实参考圈，再选择或编辑区间。编辑时可在走线预览上依次点击起点和终点，也可输入距离微调；走线支持缩放、平移和双击复位。四项指标显示区间耗时、最低速度、制动起点和恢复油门；证据不足的位置显示为「—」。点击差异说明可返回速度曲线定位。编辑器和输入曲线按需展开或切换，关闭编辑器或切换页签不会清空未保存输入；切换赛道、参考对象或离开页面后应重新核对。
