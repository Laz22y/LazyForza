...
- 当前客户端：`1.5.2`
- 当前 RaceServer：`0.5.0`
- 客户端预览版：[`1.5.3-alpha-3 · Radio Check`](https://github.com/Laz22y/LazyForza/releases/tag/v1.5.3-alpha-3)
...
`1.5.3-alpha-3 · Radio Check` 为当前预览版，提供弯道分析、语音工程师、品牌顶栏、侧栏快速设置和设置即时应用。地产赛事推荐配合 RaceServer `0.6.0-alpha-1`，使用协议 v2。预览版通过独立通道更新，当前正式版为 `1.5.2`。

圈速分析默认显示最近有效圈。展开「选择对比圈」，勾选最多四圈后点击「查看对比」；等级筛选只列出有记录的等级。「曲线」「走线」「弯道」页签切换不同分析视图，速度与输入图表带有距离及数值刻度。删除操作统一位于「管理圈速记录」，仍需确认；导入导出和实时分段位于页面下方的折叠区域。

在「弯道」页签选择分析圈与真实参考圈，再选择或编辑区间。四项指标显示区间耗时、最低速度、制动起点和恢复油门；证据不足的位置显示为「—」。点击差异说明可返回速度曲线定位。编辑器和输入曲线按需展开或切换，关闭编辑器或切换页签不会清空未保存输入；切换赛道、参考对象或离开页面后应重新核对。
...
侧栏底部显示遥测状态和正在监听的 UDP 端口。「设置 → 常规 → 快速设置」可选择语音静音、音量和减少动态，默认显示语音静音。短窗口使用紧凑图标入口。
...
RaceServer `0.5.0` 推荐搭配 LazyForza `1.5.2`，并兼容 `1.4.2–1.5.1` 的协议 v2 主要赛事流程。断线计圈恢复需要客户端 `1.4.8` 或更高版本，并由服务端总控主动开启。旧版仍可连接，但不会拥有后续新增的全部能力。
